...
import { CANDOR_SEPOLIA } from '../utils/candor';
...
async function main() {
  ...
  const candor = await getContractAt('Candor', CANDOR_SEPOLIA);
  ...
  const tx = await candor.connect(altAccount).attestReview(TEST_UUID_STRING, encodedReview, encodedProof);
